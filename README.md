...
- **Caching** — SQLite tiers: long-lived coordinate→grid metadata (30 days,
  ...
  last good forecast is served rather than an error. A **negative ("not-found")
  cache** remembers forecast `404`s — a marine/coastal cell answering
  `MarineForecastNotSupported` is the canonical case — for a longer, structurally
  stable TTL, so the area fan-out and the background warmer stop re-requesting
  cells NWS does not cover (`NotFoundForecastTtl`, default 6 h; `0` disables it).
...
- **Observability** — Aspire `ServiceDefaults` set up OpenTelemetry, health
  ...
  hit-rate (including negative-cache hits) and NWS latency/throttling. It all
  flows into the Aspire dashboard.
...
- **Weather.Infrastructure.Tests** — the NWS client (parsing, conditional GET,
  failure mapping, RFC 7807 problem parsing) against a stub handler; the SQLite
  caches (including the forecast negative cache) against a real temp database;
  the full `WeatherService` cache-aside policy, including the 404 negative-cache
  skip/write/clear; the background warmer.
...
throttling, and errors of every NWS call. Negative-cache hits show up on the
`weather.cache.requests` counter as `cache="forecast.notfound"`, and each newly
remembered 404 increments `weather.nws.notfound` tagged by problem `type` (e.g.
`MarineForecastNotSupported`) — so the marine 404s you'd otherwise see repeating
in traces become a single, explainable signal. See
